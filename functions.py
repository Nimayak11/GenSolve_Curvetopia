...
# [All other functions remain the same until show_regular]
...
# [Other functions remain the same until show_denoise]
...
def verify_accuracy(dir_regular, dir_low_noise, dir_high_noise, image_type='.jpg'):
    ...
    for img in imgs:
        ...
        # Save the plot instead of showing it
        plt.savefig(f'{shape_detect_title}_{img_edited}.png')
        plt.close()  # Close the figure to free up memory

# [The rest of the demo functions remain the same, just replace plt.show() with plt.savefig() and plt.close() as shown above]
